...
struct _ym_fft_float {
    FFTSetup setup;
    float *buffer;
};

struct _ym_fft_double {
    FFTSetupD setup;
    double *buffer;
};

static void _ym_fft_float_dealloc(ym_fft_float_ref fft) {
    if (fft->setup) vDSP_destroy_fftsetup(fft->setup);
    if (fft->buffer) free(fft->buffer);
}

static void _ym_fft_double_dealloc(ym_fft_double_ref fft) {
...
ym_fft_float_ref ym_fft_create_float(uint32_t length) {
    ...
    ym_fft_float_ref fft = ym_alloc(struct _ym_fft_float, _ym_fft_float_dealloc);
    if (fft) {
        uint32_t log2n = log2f((float)length);
        fft->setup = vDSP_create_fftsetup(log2n, FFT_RADIX2);
        fft->buffer = malloc(2 * length * sizeof(float));
        if (fft->setup == NULL || fft->buffer == NULL) {
            _ym_fft_float_dealloc(fft);
            fft = NULL;
        }
    }
    return fft;
...
ym_fft_double_ref ym_fft_create_double(uint32_t length) {
    ...
    ym_fft_double_ref fft = ym_alloc(struct _ym_fft_double, _ym_fft_double_dealloc);
    if (fft) {
        uint32_t log2n = log2f((float)length);
        fft->setup = vDSP_create_fftsetupD(log2n, FFT_RADIX2);
        fft->buffer = malloc(2 * length * sizeof(double));
        if (fft->setup == NULL || fft->buffer == NULL) {
            _ym_fft_double_dealloc(fft);
            fft = NULL;
        }
    }
    return fft;
...
void ym_fft_cpx_float(ym_fft_float_ref setup,
                      ...
                      uint32_t         length) {
    uint32_t log2n = log2f((float)length);
    DSPSplitComplex tmp = { setup->buffer, setup->buffer + length };
    vDSP_ctoz((DSPComplex *)input, 2, &tmp, 1, length);
    vDSP_fft_zip(setup->setup, &tmp, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    vDSP_ztoc(&tmp, 1, (DSPComplex *)output, 2, length);
}
...
void ym_fft_cpx_double(ym_fft_double_ref setup,
                       ...
                       uint32_t          length) {
    uint32_t log2n = log2f((float)length);
    DSPDoubleSplitComplex tmp = { setup->buffer, setup->buffer + length };
    vDSP_ctozD((DSPDoubleComplex *)input, 2, &tmp, 1, length);
    vDSP_fft_zipD(setup->setup, &tmp, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    vDSP_ztocD(&tmp, 1, (DSPDoubleComplex *)output, 2, length);
}
...
void ym_fft_cpx_split_float(ym_fft_float_ref    setup,
    ...
    uint32_t log2n = log2f((float)length);
    if (input == output) {
        vDSP_fft_zip(setup->setup, (DSPSplitComplex *)input, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    } else {
        vDSP_fft_zop(setup->setup, (DSPSplitComplex *)input, 1, (DSPSplitComplex *)output, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    }
}
...
void ym_fft_cpx_split_double(ym_fft_double_ref    setup,
    ...
    uint32_t log2n = log2f((float)length);
    if (input == output) {
        vDSP_fft_zipD(setup->setup, (DSPDoubleSplitComplex *)input, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    } else {
        vDSP_fft_zopD(setup->setup, (DSPDoubleSplitComplex *)input, 1, (DSPDoubleSplitComplex *)output, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    }
}
...
void ym_fft_real_float(ym_fft_float_ref setup,
                       ...
                       uint32_t         length) {
    uint32_t log2n = log2f((float)length);
    DSPSplitComplex tmp = { setup->buffer, setup->buffer + length/2 };
    vDSP_ctoz((DSPComplex *)input, 2, &tmp, 1, length / 2);
    vDSP_fft_zrip(setup->setup, &tmp, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    if (direction == YM_FFT_FORWARD) {
        float scale = 0.5;
        vDSP_vsmul(setup->buffer, 1, &scale, setup->buffer, 1, length);
    }
    vDSP_ztoc(&tmp, 1, (DSPComplex *)output, 2, length / 2);
}
...
void ym_fft_real_double(ym_fft_double_ref setup,
                        ...
                        uint32_t          length) {
    uint32_t log2n = log2f((float)length);
    DSPDoubleSplitComplex tmp = { setup->buffer, setup->buffer + length/2 };
    vDSP_ctozD((DSPDoubleComplex *)input, 2, &tmp, 1, length / 2);
    vDSP_fft_zripD(setup->setup, &tmp, 1, log2n, direction == YM_FFT_INVERSE ? FFT_INVERSE : FFT_FORWARD);
    if (direction == YM_FFT_FORWARD) {
        double scale = 0.5;
        vDSP_vsmulD(setup->buffer, 1, &scale, setup->buffer, 1, length);
    }
    vDSP_ztocD(&tmp, 1, (DSPDoubleComplex *)output, 2, length / 2);
}
 ...
 */
static void passf2_ps(int ido, int l1, const v4sf *cc, v4sf *ch, const float *wa1, float fsign) { //never inline
    int k, i;
    int l1ido = l1*ido;
    if (ido <= 2) {
 ...
 */
static void passf3_ps(int ido, int l1, const v4sf *cc, v4sf *ch, const float *wa1, const float *wa2, float fsign) { //never inline
    static const float taur = -0.5f;
    float taui = 0.866025403784439f /*sin 60°*/ * fsign;
    int i, k;
...
static void passf4_ps(int ido, int l1, const v4sf *cc, v4sf *ch, const float *wa1, const float *wa2, const float *wa3, float fsign) { //never inline
 ...
 */
static void passf5_ps(int ido, int l1, const v4sf *cc, v4sf *ch, const float *wa1, const float *wa2, const float *wa3, const float *wa4, float fsign) { //never inline
    static const float tr11 =  0.309016994374947f;          // sin 18°
           const float ti11 =  0.951056516295154f * fsign;  // sin 72°
    static const float tr12 = -0.809016994374947f;          // sin-54°
           const float ti12 =  0.587785252292473f * fsign;  // sin 36°
...
static void radf2_ps(int ido, int l1, const v4sf * restrict cc, v4sf * restrict ch, const float *wa1) { //never inline
...
static void radb2_ps(int ido, int l1, const v4sf *cc, v4sf *ch, const float *wa1) { //never inline
...
static void radf3_ps(int ido, int l1, const v4sf * restrict cc, v4sf * restrict ch, const float *wa1, const float *wa2) {
    static const float taur = -0.5f;
    static const float taui = 0.866025403784439f; // sin 60°
    int i, k, ic;
...
static void radb3_ps(int ido, int l1, const v4sf *restrict cc, v4sf *restrict ch, const float *wa1, const float *wa2) {
    static const float taur = -0.5f;
    static const float taui =   0.866025403784439f;   // sin 60°
    static const float taui_2 = 0.866025403784439f*2; // sin 60°
    int i, k, ic;
...
static void radf4_ps(int ido, int l1, const v4sf *restrict cc, v4sf * restrict ch, const float * restrict wa1, const float * restrict wa2, const float * restrict wa3) { //never inline
    static const float minus_hsqt2 = (float) -0.7071067811865475;  // -sqrt(2)/2
    int i, k, l1ido = l1*ido;
    {
...
static void radb4_ps(int ido, int l1, const v4sf * restrict cc, v4sf * restrict ch, const float * restrict wa1, const float * restrict wa2, const float *restrict wa3) { //never inline
    static const float minus_sqrt2 = (float) -1.414213562373095; // -sqrt(2)
...
static void radf5_ps(int ido, int l1, const v4sf * restrict cc, v4sf * restrict ch, const float *wa1, const float *wa2, const float *wa3, const float *wa4) {
    static const float tr11 =  0.309016994374947f; // sin 18°
    static const float ti11 =  0.951056516295154f; // sin 72°
    static const float tr12 = -0.809016994374947f; // sin-54°
    static const float ti12 =  0.587785252292473f; // sin 36°
...
static v4sf * rfftf1_ps(int n, const v4sf *input_readonly, v4sf *work1, v4sf *work2, const float *wa, const int *ifac) { //never inline
...
static v4sf * rfftb1_ps(int n, const v4sf *input_readonly, v4sf *work1, v4sf *work2, const float *wa, const int *ifac) { //never inline
    v4sf *in  = (v4sf*)input_readonly;
    v4sf *out = (in == work2 ? work1 : work2);
    int nf = ifac[1], k1;
    int l1 = 1;
...
PFFFT_Setup *pffft_new_setup(int N, pffft_transform_t transform) {
    ...
    if (m != N/SIMD_SZ) {
        void pffft_destroy_setup(PFFFT_Setup *s); /// delete！！！
...
void pffft_cpx_finalize(int Ncvec, const v4sf *in, v4sf *out, const v4sf *e) {
...
void pffft_cpx_preprocess(int Ncvec, const v4sf *in, v4sf *out, const v4sf *e) {
    int k, dk = Ncvec/SIMD_SZ; // number of 4x4 matrix blocks
    v4sf r0, i0, r1, i1, r2, i2, r3, i3;
    v4sf sr0, dr0, sr1, dr1, si0, di0, si1, di1;
    assert(in != out);
...
static inline void pffft_real_finalize_4x4(const v4sf *in0, const v4sf *in1, const v4sf *in, const v4sf *e, v4sf *out) {
    v4sf r0, i0, r1, i1, r2, i2, r3, i3;
    v4sf sr0, dr0, sr1, dr1, si0, di0, si1, di1;
    r0 = *in0; i0 = *in1;
...
static void pffft_real_finalize(int Ncvec, const v4sf *in, v4sf *out, const v4sf *e) { //never inline
    int k, dk = Ncvec/SIMD_SZ; // number of 4x4 matrix blocks
...
static inline void pffft_real_preprocess_4x4(const v4sf *in, const v4sf *e, v4sf *out, int first) { //never inline
...
static void pffft_real_preprocess(int Ncvec, const v4sf *in, v4sf *out, const v4sf *e) { //never inline
    int k, dk = Ncvec/SIMD_SZ; // number of 4x4 matrix blocks
    /* fftpack order is f0r f1r f1i f2r f2i ... f(n-1)r f(n-1)i f(n)r */
...
/**
 ...
 @param finput    <#finput description#>
 @param foutput   <#foutput description#>
 @param scratch   <#scratch description#>
 @param direction <#direction description#>
 @param ordered   <#ordered description#>
 */
void pffft_transform_internal(PFFFT_Setup *setup, const float *finput, float *foutput, v4sf *scratch, pffft_direction_t direction, int ordered) {
    ...
    v4sf scratch_on_stack[stack_allocate];
    ...
    if (direction == PFFFT_FORWARD) {
        ...
        if (setup->transform == PFFFT_REAL) {
            ...
            pffft_cpx_finalize(Ncvec, buff[ib], buff[!ib], (v4sf*)setup->e);
        }
        if (ordered) {
    ...
    /// ifft
        ...
        if (setup->transform == PFFFT_REAL) {
            ...
            pffft_cpx_preprocess(Ncvec, vinput, buff[ib], (v4sf*)setup->e);
